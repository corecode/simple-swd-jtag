...
/* PLL1 Setting Check */
  ...
  #define PLL1_FCCO ((2 * 2^(PLL1_PSEL + 1)) * (PLL1_MSEL + 1) * (PLL1_CLKIN / (PLL1_NSEL + 1)))
  #define PLL1_FOUT ((PLL1_MSEL + 1) * (PLL1_CLKIN / (PLL1_NSEL + 1)))
 #else                                  /* Noninteger Mode                    */
  #define PLL1_FCCO ((PLL1_MSEL + 1) * (PLL1_CLKIN / (PLL1_NSEL + 1)))
  #define PLL1_FOUT (PLL1_FCCO / (2 * 2^(PLL1_PSEL + 1)))
 #endif
...
#if (USE_SPIFI)
  /* Copy vectors from SPIFI to RAM */
  src = (uint32_t *)0x14000000;
